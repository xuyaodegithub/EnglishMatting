<template>
    <div class="helloFirst">
        ...
        <div class=" apps">
            <div class="Operator">
                <h6><!--Local images-->Images on Local Devices</h6>
                <el-button type="primary" round icon="el-icon-upload2" @click="upLoadimg()"><!--Upload-->Upload</el-button>
                <p class="afterbtn" v-if="!LoginStatus">Batch upload after <span class="cu" @click="userlogin(0)">Sign in</span>
                    <!--for batch upload--> </p>
                <div class="center">
                    <h5>Web Images</h5>
...
<!--                        @keyup.enter="copyImgUrl()"-->
                        <input type="text" placeholder="Paste (CTRL+V) image or URL" v-model="imgUrl"
...
<!--                    </div>-->
                ...
                <div class="bottonLast">
                    ...
                    <div class="flex a-i cu" @click="openHistory()">
                        ...
                        <span><!--History-->Use Histories</span>
                    </div>
                    <div class="fixedB">
                        <h5><!--Contact us-->Contact Us</h5>
                        <a href="https://www.facebook.com/PicupAI-110303597147629/" target="_blank">
                            <div class="flex a-i cu">
    ...
    <!--                            <img src="../../assets/image/wxl.png" alt="" class="noC">-->
            ...
            <div class="OperatorRight drops">
                <div class="newChunk">
                    <h2>Portrait Cutout</h2>
                    <p>Single or multiple people, front and side views</p>
                    <div class="flex a-i">
                        <div class="gif"><img src="../../assets/image/pgif.gif" alt=""></div>
                        <div class="gifright">
                            <el-button type="primary" round icon="el-icon-upload2" @click="upLoadimg()"><!--Upload-->Upload Image</el-button>
                            <el-input v-model="imgUrl" class="upcas" placeholder="Paste (CTRL+V) image or URL"
                                      @focus="$event.target.select()"></el-input>
                            <p>No pictures? Try the pictures below to see the effect</p>
                            <div class="flex a-i">
...
<!--                </el-scrollbar>-->
                <div class="downBtn" @mouseenter="classType=0">
                    <span><!--Download all--> <i class="el-icon-download"></i><br> all</span>
                    <div class="sizeChoses">
                        ...
                        <div class="flex a-i">
                            ...
                            <span>0</span>
                            <span class="cu" @click="saveMove(0)">Download</span>
                        </div>
                        <div class="flex a-i ">
                            <span>Download original</span>
                            <span>{{allbgImg.length}}</span>
                            <span class="cu" @click="saveMove(1)">Download</span>
                        </div>
                        <div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                initimgsList:[
                    ...
                    'http://deeplor.oss-cn-hangzhou.aliyuncs.com/matting_original/2019/11/15/033cbfc0c33448e4b703c7d39c204934.png',
                ],
                rightImgList: [],
                selectImg: 0,
                multiple: false,
        ...
        filters: {
            ...
            imgtitle(i){
                if (i===0)return 'Background transparency';
                else if (i===1)return 'Pure color background';
                else if (i===2)return 'Background black and white';
                else if (i===3)return 'Background ambiguity';
            }
        },
        watch:{
            // files: {
        ...
        methods: {
            ...
            saveMove(key) {
                ...
                if (this.files.length !== this.allbgImg.length) {
                    this.$message( {type: 'warning', message: 'Processing, please wait...'} );
                    return
                }
                ...
                let callback = (objs) => {
                    ...
                    if (baseList.length === arr.length) {
                        if (baseList.length > 0) {
                            ...
                            _this.$message.error( {
                                title: "error",
                                message: "No pictures to download"
                            } );
                        }
                    }
                }
            ...
            copyImgUrl(url,k) {//粘贴图片链接确定
                if (!this.imgUrl && !url) {
                    this.$message( {
                        message: 'Picture address cannot be empty',
                        type: 'warning'
                    } );
                    return
                }
            ...
            changeImg(ev) {//图片上传
                ...
                if(e.files.length>30){
                    this.$message({
                        type:'warning',
                        message:'Upload up to 30 pictures at the same time, please upload in batches.'
                    })
                    return
                }
                this.toscroll();
                ...
                for(let i=0;i<oDrops.length;i++){
                    oDrops[i].addEventListener("drop", function (e) {
                        ...
                        if(!getToken() && files.length>1) {
                            _self.$message({type:'error',message:'Batch upload after login'})
                            return
                        }
                        _self.toscroll();
