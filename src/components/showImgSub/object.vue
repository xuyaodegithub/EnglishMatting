        ...
        <div class="dreepAfter">
            ...
            <div class="flex imgs">
                <div>
                    ...
                    <div class="bg_img flex" v-show="showBgList">
                        <div class="title flex a-i">
                            ...
                            <input type="file" name="file" accept="image/*" :multiple="false" ref="selfImg" @change="changeselfImg" style="display: none;">
                            <el-button round icon="el-icon-upload2" style="margin-left:30px;font-size: 12px;line-height: 28px;border-color: #21a9e8;padding: 0 8px;color: #21a9e8;" @click="upselfImg">Custom background</el-button>
                            <i class="el-icon-close cu" style="right: 0;" @click="showBgList=false"></i>
                        </div>
                        <div class="flex bgType a-i">
                            <span class="cu" v-for="(item,idx) in bgclass" :key="idx" :class="{active : bgType===idx}" @click="changeBgType(idx)">{{item}}</span>
                        </div>
                        <div class="flex a-i bgimgs f-w">
                            <div v-for="(item,idx) in oneItemBg" :key="idx" class="cu" :class="{active : selectIdx===idx}" @click="selectBg(item,idx)">
                                <img :src="item" alt="">
                            </div>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div v-show="bgOriginal.status===0" :style="backg" class="activeDiv" ref="activeImg" @mousedown="down">
                        <canvas :id="'canvas'+ramdId" >This function is not supported by the current Tourer. Try another one!</canvas>
                        <canvas :id="'canvas'+index" class="bgss">This function is not supported by the current Tourer. Try another one!</canvas>
                        <div class="img" ref="oIImg" :style="{backgroundImage:`url(${scale})`}" v-show="!canDown && choseBack==='bg'">
                            <!--                            <img src="../../assets/image/scale.png" alt="" style="border-radius: 0">-->
                        </div>
                        <img src="../../assets/image/mmmmm.png" alt="" ref="mmmmm" v-show="choseBack==='bg' && initfirst">
                    </div>
                    <div v-show="bgOriginal.status===1" class="errmsg">
                        <i class="el-icon-circle-close"></i>
                        This picture is not supported at present and cannot recognize the foreground
                        <!--                        Error occured, the foreground can not be recognized-->
                        <p>
                        	Please select a picture with a clear distinction between foreground and background. For example: try a picture of one person, a product, an animal, a car, or another object
                            <!--                        	Try picture that contains person, more categories will be supported in future-->
                        </p>
                    </div>
                    <div v-show="bgOriginal.status===3" class="errmsg">
                        <i class="el-icon-circle-close"></i>
                        The picture is too large to handle temporarily
                        <!--                        Error occured, the foreground can not be recognized-->
                        <p>
                            Please select a picture no more than 15m for processing
                            <!--                        	Try picture that contains person, more categories will be supported in future-->
                        </p>
                    </div>
                    <div v-show="bgOriginal.status===2" class="errmsg">
                        <i class="el-icon-s-flag"></i>
                       Currently in queue, please wait...
                        <!--                        Waiting in the queue,-->
                        <p>
                            Queuing position：{{imageMsg.queueNumber}}
                    ...
                    <div class="otherBtn flex a-i"  v-if="bgOriginal.img">
                        ...
                        <div v-for="(items,indexs) in color" :key="indexs"
                            ...
                            <div class="flex color_List" v-show="showcolorList && indexs===1">
                                <span v-for="(color,idx) in colorList" :key="idx" :style="{backgroundColor:color}" @click.stop="choseColor(color)"></span>
                            </div>
                            <img :src="items" alt="" style="position: absolute;left: 0;top: 0;width: 28px;height: 28px;border-radius: 50%;" :title="indexs | imgtitle">
                        </div>
                        <span class="el-icon-picture-outline cu bg_img" @click="showBgList=!showBgList" :class="{'bordershow': choseBack=='bg'}">Change background</span>
                        <i class="el-icon-more cu" style="font-size: 28px;color: #999;position: relative;margin-left: auto;">
                            <el-checkbox v-model="offNo" style="position: absolute;top: 100%;left: -200%;" @change="setInitStatus">Set as default background</el-checkbox>
                        </i>
                    ...
                    <div class="flex">
                        <down-btn  v-if="bgOriginal.img" :imageMsg="imageMsg" @edireThis="edireThis" @save="save" :type="2"></down-btn>
                        <down-btn  v-if="bgOriginal.img" :imageMsg="imageMsg" @edireThis="edireThis" @save="save" :type="2" down></down-btn>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                loadImg:'',
                subs:0,
                bgclass:['Photography','Hand drawn','Color'],
                showBgList:false,
                bgType:0,
                selectIdx:-1,
                bgobj:'',
        ...
        methods: {
            setInitStatus(e){//s设置初始化状态
                ...
                if(this.offNo){
                    localStorage.setItem('initObjStatus',JSON.stringify(sObj))
                    this.$store.commit('SET_INIT_STATUS',sObj)
                } else {
                    localStorage.removeItem('initObjStatus')
            ...
            down(e) {//鼠标按下
                ...
                this.downxy = {x: (ev.clientX - this.oCantl.left)*this.canveaContentW/this.oDDiv.offsetWidth, y: (ev.clientY - this.oCantl.top)*this.canveaContentH/this.oDDiv.offsetHeight}//每次鼠标按下的位置
                const bigit = this.canvasText.isPointInPath( this.downxy.x, this.downxy.y ), smallit = this.initfour( ev.clientX - this.oCantl.left, ev.clientY - this.oCantl.top );
                if (!smallit && bigit) {
                    this.canDown = true
            ...
            ups(e) {//鼠标弹起
                if(this.choseBack!=='bg')return;
                const ev = e || window.event;
                const upxy = {x: (ev.clientX - this.oCantl.left)*this.canveaContentW/this.oDDiv.offsetWidth, y: (ev.clientY - this.oCantl.top)*this.canveaContentH/this.oDDiv.offsetHeight}
                if (this.canDown) {
                    this.canDown = false
                    let initw,inith
            ...
            moves(e) {//鼠标移动
                if(this.choseBack!=='bg')return;
                const ev = e || window.event;
                const movexy = {x: (ev.clientX - this.oCantl.left)*this.canveaContentW/this.oDDiv.offsetWidth, y: (ev.clientY - this.oCantl.top)*this.canveaContentH/this.oDDiv.offsetHeight}
            ...
            updataThis() {
                if (this.bgOriginal.status !== 0) {//当前图片不可编辑
                    this.$message({type: 'error', message: 'The current picture is not editable'})
                    return
                }
            ...
            save(index,e) {//保存下载
                if (index === 0) {
                    ...
                    this.initSmallTag( e,'Free Admission :）' )
                    this.downOldImg(url)
                } else {
                    if (this.imageMUrl) {
                        this.initSmallTag( e,'Free Admission :）' )
...
<style lang="scss">
            ...
            & > div:first-child{
                ...
                .bg_img{
                    ...
                    .title{
                        margin-bottom: 12px;
                        .el-button:hover{
                            background-color: rgba(33, 169, 232, .3);
                        }
                    }
    ...
    .otherBtn {
        ...
        i .el-checkbox__input.is-checked + .el-checkbox__label{
            color: $to;
        }
